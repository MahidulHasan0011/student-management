  ...
  async toggleAdmissionTest(req, res, next) {
    try {
      ...
      if (admission_test_enabled === undefined) {
        return errorResponse(res, { message: "admission_test_enabled is required", statusCode: 400 });
      }
      const data = await academicSessionService.toggleAdmissionTest(req.params.id, admission_test_enabled);
