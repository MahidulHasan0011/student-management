const examResultsRepository = require('./result.repository');
...
const getResults = async (queryOptions) => {
  ...
  const config = {
    searchableColumns: ['st.full_name', 'sub.name', 'e.name', 'er.grade'],

    filterableColumns: ['er.exam_id', 'er.student_id', 'er.subject_id', 'er.grade'],
  };
  const whereClause = buildWhereClause(queryOptions, values, config, countRef, 'er');
